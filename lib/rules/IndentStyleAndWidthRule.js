...
IndentStyleAndWidthRule = {};

IndentStyleAndWidthRule.name = 'indent_style_and_width';

var postfixExpressions = [
    'LogicalExpression',
    'BinaryExpression',
    'UnaryExpression'
];

IndentStyleAndWidthRule.prepare = function (tree, contents ) {
...
IndentStyleAndWidthRule.infer = function (sample, callback) {
    var characters = {},
        indent = 0,
        previousToken = null,
        totalCount = 0;
    ...
    sample.on('end', function () {
        ...
        for (var character in characters) {
            characters[character].forEach(fn, character);
        }
...
IndentStyleAndWidthRule.transform = function (input, value, output) {
    ...
    assert(
        ( value.indent_style === 'space' || value.indent_style === 'tab' ) &&
        ( typeof value.indent_size === 'number' && value.indent_size >= 0 )
    );

    var prevToken = null,
        indentChar = value.indent_style === 'tab' ? '\t' : ' ';
...
//          prevToken.value = '\n'.repeat(lineCount) + indentChar.repeat( indent * value.indent_size ) + '/*' + (indent) + '(' + path + ')*/ ';
            prevToken.value = '\n'.repeat(lineCount) + indentChar.repeat( indent * value.indent_size );
...
module.exports = IndentStyleAndWidthRule;
